...
using Snowflake = snowflake<1735689600000LL, std::mutex>;

// 提供静态方法生成ID
struct SnowflakeID {
    static int64_t generate(int64_t workerid = 1, int64_t datacenterid = 1) {
        static Snowflake generator;
        static bool initialized = false;
        if (!initialized) {
            // 可根据机器配置
            generator.init(workerid, datacenterid);
            initialized = true;
        }
